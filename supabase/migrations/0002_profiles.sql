-- ============================================================
-- 0002: Profiles, onboarding, auth triggers
-- ============================================================
...
do $$ begin
  ...
  if not exists (select 1 from pg_policies where tablename = 'profiles' and policyname = 'users update own profile') then
    execute 'create policy "users update own profile" on profiles for update
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename = 'onboarding' and policyname = 'users manage own onboarding') then
    execute 'create policy "users manage own onboarding" on onboarding for all
