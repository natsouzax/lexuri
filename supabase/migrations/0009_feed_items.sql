...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'feed_items' AND policyname = 'public read active feed items'
  ) THEN
    EXECUTE 'CREATE POLICY "public read active feed items"
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'feed_items' AND policyname = 'service role manages feed items'
  ) THEN
    EXECUTE 'CREATE POLICY "service role manages feed items"
...
INSERT INTO feed_items (id, type, title, channel, artist, youtube_id, duration, level, tags, preview) VALUES
  ...
  ('music-roar', 'music', 'Roar', null, 'Katy Perry', 'VFA3nPRYU98', '3:43', 'A2', array['pop','empowerment','idioms'], 'Empowerment anthem with clear, strong lyrics. Introduces common English metaphors and idioms about strength and standing up for yourself.'),
